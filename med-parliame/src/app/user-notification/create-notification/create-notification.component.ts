...
export class CreateNotificationComponent implements OnInit {
  ...
  UpdateNews(){
    ...
    const newsData = {
      ...
      Desc: this.frmNews.get('newsDesc').value,
      UserId : this.frmNews.get('userCreate').value,
      UserType: this.frmNews.get('userTypeId').value,
      Id: this.frmNews.get('id').value,
      
    };

    const formData = new FormData();
    formData.append('NotificationBanner', this.frmNews.get('banner').value ? this.frmNews.get('banner').value : 'NULL');
    formData.append('data', JSON.stringify(newsData));

    console.log(formData);
    this.apiService.dataPostApi(formData, AppSettings.adduserNotificationUpdate).then((data: any[]) => {
      console.log(data);
      if(data['status'] == 'true'){
        this.frmNews.reset();
  ...
  submitNews() {
    ...
    console.log(formData);
    this.apiService.dataPostApi(formData, AppSettings.adduserNotification).then((data: any[]) => {
      console.log(data);
      if(data['status'] == 'true'){
        this.frmNews.reset();
        jQuery('#addAdmin-news11').modal('show')
  ...
  getNewsData(newsId){
    const params = { Id: newsId};
      this.apiService.dataPostApi(params,AppSettings.getUserNotification).then((data: any[]) => {
...
// this.frmNews.get('videoLink').clearValidators();
    ...
    // this.frmNews.get('newsType').setValue(this.newsDetails[0]['UserType']);
    ...
    //   })
    // }
    this.frmNews.get('userTypeId').setValue(this.newsDetails[0]['UserType'])
  }
  removeImage(){
    this.removeButton = false;
