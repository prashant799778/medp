...
this.userTypeDetails = resp['result']
  ...
  editNews(NewsId) {
    console.log(NewsId);
    this.router.navigate(['/userNotification/createNotification'], { queryParams: {NewsId: NewsId}});
  }
  ...
  deletedNewss(){
    ...
    this.apiService.dataPostApi(data, AppSettings.deleteUserNotification).then((data: any[]) => {
      console.log(data);
      if(data['status'] == 'true'){
