...
export class CreateInfoComponent implements OnInit {
  ...
  UpdateNews(){
    ...
    this.apiService.dataPostApi(formData, AppSettings.signUpVideo).then((data: any[]) => {
      console.log(data);
      if(data['status'] == 'true'){
        this.frmNews.reset();
    ...
    const params = { id: newsId};
      this.apiService.dataPostApi(params,AppSettings.getSignUpVideo).then((data: any[]) => {
        ...
        if (data['status'] === 'true') {
          ...
          if(this.newsDetails[0].videoLink == '' && this.newsDetails[0].imagePath == ''){
            this.videoTextCheck = true;
